...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-surface rounded-2xl p-6 shadow-sm border border-gray-100 flex items-center gap-4 border-l-4 border-primary">
            ...
            <div>
                ...
                <h3 class="text-xl font-bold text-gray-800">Rp {{ number_format($kpi['total_pemasukan'] ?? 0, 0, ',', '.') }}</h3>
            </div>
        </div>
        ...
        <div class="bg-surface rounded-2xl p-6 shadow-sm border border-gray-100 flex items-center gap-4 border-l-4 border-red-500">
            ...
            <div>
                ...
                <h3 class="text-xl font-bold text-gray-800">Rp {{ number_format($kpi['total_tunggakan'] ?? 0, 0, ',', '.') }}</h3>
            </div>
        </div>
        ...
        <div class="bg-surface rounded-2xl p-6 shadow-sm border border-gray-100 flex items-center gap-4 border-l-4 border-green-500">
            ...
            <div>
                ...
                <h3 class="text-xl font-bold text-gray-800">{{ number_format($kpi['siswa_lunas'] ?? 0) }} <span class="text-sm font-normal text-gray-500">Siswa</span></h3>
            </div>
        </div>
        ...
        <div class="bg-surface rounded-2xl p-6 shadow-sm border border-gray-100 flex items-center gap-4 border-l-4 border-yellow-500">
            ...
            <div>
                ...
                <h3 class="text-xl font-bold text-gray-800">{{ number_format($kpi['siswa_nunggak'] ?? 0) }} <span class="text-sm font-normal text-gray-500">Siswa</span></h3>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-surface p-6 rounded-2xl shadow-sm border border-gray-100">
            <h3 class="text-lg font-bold text-gray-800 mb-4">Grafik Pemasukan Tahun {{ $tahunIni ?? date('Y') }}</h3>
...
<script>
    ...
    // 2. Render Chart Pemasukan (Bar Chart Clean)
        ...
        new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    borderWidth: 1,
                    borderRadius: 4,
                    barPercentage: 0.6 // Membuat batang tidak terlalu gemuk
                }]
            },
            options: {
                responsive: true,
                    ...
                    tooltip: {
                        callbacks: {
                            label: function(context) {
                                // Hanya mengembalikan angka bersih (contoh: 1.500.000)
                ...
                scales: {
                    y: {
                        beginAtZero: true,
                        suggestedMax: 1000000, // Menjaga skala chart tetap rapi (0 - 1M) meski data masih kosong
                        grid: { borderDash: [4, 4], color: '#E5E7EB' },
                        ticks: {
                            precision: 0, // Mencegah munculnya angka desimal aneh
                            callback: function(value) {
                                // Format angka singkatan murni tanpa Rp
                                if (value === 0) return '0';
