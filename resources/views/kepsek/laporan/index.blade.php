...
<div class="space-y-6">
    ...
    <div class="bg-surface p-2 rounded-2xl shadow-sm border border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4">
        ...
        <div class="px-4">
            <a href="{{ route('kepsek.laporan.cetak', request()->all()) }}" target="_blank" class="inline-flex items-center text-white bg-[#8B0000] hover:bg-[#660000] font-bold text-sm px-5 py-2.5 rounded-lg transition-colors shadow-sm">
                <i class="fa-solid fa-print mr-2"></i> Cetak Laporan Resmi
            </a>
        </div>
    </div>

    <form action="{{ route('kepsek.laporan.index') }}" method="GET" class="bg-surface p-5 rounded-2xl shadow-sm border border-gray-100">
